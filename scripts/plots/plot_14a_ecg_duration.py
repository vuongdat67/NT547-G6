...
CSV_PATH = "artifacts/onchain/repeated_onchain_runs.csv"
...
df['lat_ma'] = df['duration_ms'].rolling(window=window_size, min_periods=window_size).mean().bfill()
df['lat_std'] = df['duration_ms'].rolling(window=window_size, min_periods=window_size).std().bfill()
...
ax1.plot(df['run_idx'], df['duration_ms'], color=C_RAW, alpha=0.6, linewidth=0.8, label="Raw Latency (ms)")
...
ax1.set_title(r"21. (a) End-to-End On-Chain Deployment Latency (ECG Plot)")
...
ax2.set_title(r"21. (b) Spend Transaction Size Volatility due to Nonces")
...
out_path = os.path.join(FIGURES_DIR, "plot_14a_ecg_duration.png")
